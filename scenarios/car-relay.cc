...
NS_LOG_COMPONENT_DEFINE ("Experiment");// define a log conponent with a specific name
...
V2vNetDeviceFaceCallback (Ptr<Node> node, Ptr<ndn::L3Protocol> ndn, Ptr<NetDevice> device)
{
  ...
  Ptr<ndn::NetDeviceFace> face = CreateObject<ndn::V2vNetDeviceFace> (node, device);// NDN face that belongs to the node and through which prefix is accessible
  ndn->AddFace (face);
...
main (int argc, char *argv[])
{
  ...
  ////////////////
  /*
	boost::tuple<> could return multi values
  */
